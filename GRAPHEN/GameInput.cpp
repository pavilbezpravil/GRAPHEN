...
#define WinRTKey(name) (unsigned char)Windows::System::VirtualKey::name
    ...
    void KbmInitialize()
    {
        ...
        if (FAILED(s_Keyboard->SetCooperativeLevel(s_Window->GetHWND(), DISCL_FOREGROUND | DISCL_NONEXCLUSIVE)))
        ...
        if (FAILED(s_Mouse->SetCooperativeLevel(s_Window->GetHWND(), DISCL_FOREGROUND | DISCL_EXCLUSIVE)))
    ...
    void KbmUpdate()
    {
        ...
        if (foreground != s_Window->GetHWND()// wouldn't be able to acquire
            || !visible)
        {
            KbmZeroInputs();
        }

...
inline void Vector2::Transform(const Vector2& v, const Matrix4& m, Vector2& result) noexcept
{
    using namespace DirectX;
    XMVECTOR v1 = XMLoadFloat2(&v);
...
inline Vector2 Vector2::Transform(const Vector2& v, const Matrix4& m) noexcept
{
    using namespace DirectX;
    XMVECTOR v1 = XMLoadFloat2(&v);
    XMMATRIX M = XMLoadFloat4x4(&m);
...
inline void Vector2::Transform(const Vector2* varray, size_t count, const Matrix4& m, Vector2* resultArray) noexcept
{
    using namespace DirectX;
    XMMATRIX M = XMLoadFloat4x4(&m);
...
inline void Vector2::Transform(const Vector2& v, const Matrix4& m, Vector4& result) noexcept
{
    using namespace DirectX;
    XMVECTOR v1 = XMLoadFloat2(&v);
    XMMATRIX M = XMLoadFloat4x4(&m);
...
_Use_decl_annotations_
inline void Vector2::Transform(const Vector2* varray, size_t count, const Matrix4& m, Vector4* resultArray) noexcept
{
    using namespace DirectX;
    XMMATRIX M = XMLoadFloat4x4(&m);
...
inline void Vector2::TransformNormal(const Vector2& v, const Matrix4& m, Vector2& result) noexcept
{
    using namespace DirectX;
    XMVECTOR v1 = XMLoadFloat2(&v);
    XMMATRIX M = XMLoadFloat4x4(&m);
...
inline Vector2 Vector2::TransformNormal(const Vector2& v, const Matrix4& m) noexcept
{
    using namespace DirectX;
    XMVECTOR v1 = XMLoadFloat2(&v);
    XMMATRIX M = XMLoadFloat4x4(&m);
...
_Use_decl_annotations_
inline void Vector2::TransformNormal(const Vector2* varray, size_t count, const Matrix4& m, Vector2* resultArray) noexcept
{
    using namespace DirectX;
    XMMATRIX M = XMLoadFloat4x4(&m);
...
inline void Vector3::Transform(const Vector3& v, const Matrix4& m, Vector3& result) noexcept
{
    using namespace DirectX;
    XMVECTOR v1 = XMLoadFloat3(&v);
    XMMATRIX M = XMLoadFloat4x4(&m);
...
inline Vector3 Vector3::Transform(const Vector3& v, const Matrix4& m) noexcept
{
    using namespace DirectX;
    XMVECTOR v1 = XMLoadFloat3(&v);
    XMMATRIX M = XMLoadFloat4x4(&m);
...
_Use_decl_annotations_
inline void Vector3::Transform(const Vector3* varray, size_t count, const Matrix4& m, Vector3* resultArray) noexcept
{
    using namespace DirectX;
    XMMATRIX M = XMLoadFloat4x4(&m);
...
inline void Vector3::Transform(const Vector3& v, const Matrix4& m, Vector4& result) noexcept
{
    using namespace DirectX;
    XMVECTOR v1 = XMLoadFloat3(&v);
    XMMATRIX M = XMLoadFloat4x4(&m);
...
_Use_decl_annotations_
inline void Vector3::Transform(const Vector3* varray, size_t count, const Matrix4& m, Vector4* resultArray) noexcept
{
    using namespace DirectX;
    XMMATRIX M = XMLoadFloat4x4(&m);
...
inline void Vector3::TransformNormal(const Vector3& v, const Matrix4& m, Vector3& result) noexcept
{
    using namespace DirectX;
    XMVECTOR v1 = XMLoadFloat3(&v);
    XMMATRIX M = XMLoadFloat4x4(&m);
...
inline Vector3 Vector3::TransformNormal(const Vector3& v, const Matrix4& m) noexcept
{
    using namespace DirectX;
    XMVECTOR v1 = XMLoadFloat3(&v);
    XMMATRIX M = XMLoadFloat4x4(&m);
...
_Use_decl_annotations_
inline void Vector3::TransformNormal(const Vector3* varray, size_t count, const Matrix4& m, Vector3* resultArray) noexcept
{
    using namespace DirectX;
    XMMATRIX M = XMLoadFloat4x4(&m);
...
inline void Vector4::Transform(const Vector4& v, const Matrix4& m, Vector4& result) noexcept
{
    using namespace DirectX;
    XMVECTOR v1 = XMLoadFloat4(&v);
    XMMATRIX M = XMLoadFloat4x4(&m);
...
inline Vector4 Vector4::Transform(const Vector4& v, const Matrix4& m) noexcept
{
    using namespace DirectX;
    XMVECTOR v1 = XMLoadFloat4(&v);
    XMMATRIX M = XMLoadFloat4x4(&m);
...
_Use_decl_annotations_
inline void Vector4::Transform(const Vector4* varray, size_t count, const Matrix4& m, Vector4* resultArray) noexcept
{
    using namespace DirectX;
    XMMATRIX M = XMLoadFloat4x4(&m);
...
inline Matrix3::Matrix3(const Matrix4& m) noexcept
{
   m_mat[0] = m.Right();
   m_mat[1] = m.Up();
   m_mat[2] = m.Backward();
...
inline Matrix3::Matrix3(const Quaternion& q) noexcept
{
   *this = Matrix3(XMMatrixRotationQuaternion(q));
}

inline Matrix3::operator XMMATRIX() const noexcept
{
   return Matrix4 { GetX(), GetY(), GetZ() };
}
...
inline bool Matrix4::Decompose(Vector3& scale, Quaternion& rotation, Vector3& translation) noexcept
{
    using namespace DirectX;
...
_Use_decl_annotations_
inline Matrix4 Matrix4::CreateBillboard(
...
_Use_decl_annotations_
inline Matrix4 Matrix4::CreateConstrainedBillboard(
    const Vector3& object,
    const Vector3& cameraPosition,
...
inline Matrix4 Matrix4::CreateTranslation(float x, float y, float z) noexcept
{
    using namespace DirectX;
    Matrix4 R;
...
inline Matrix4 Matrix4::CreateFromAxisAngle(const Vector3& axis, float angle) noexcept
{
    using namespace DirectX;
    Matrix4 R;
...
inline Matrix4 Matrix4::CreatePerspectiveFieldOfView(float fov, float aspectRatio, float nearPlane, float farPlane) noexcept
{
    using namespace DirectX;
    Matrix4 R;
...
inline Matrix4 Matrix4::CreatePerspective(float width, float height, float nearPlane, float farPlane) noexcept
{
    using namespace DirectX;
    Matrix4 R;
...
inline Matrix4 Matrix4::CreatePerspectiveOffCenter(float left, float right, float bottom, float top, float nearPlane, float farPlane) noexcept
{
    using namespace DirectX;
    Matrix4 R;
...
inline Matrix4 Matrix4::CreateOrthographic(float width, float height, float zNearPlane, float zFarPlane) noexcept
{
    using namespace DirectX;
    Matrix4 R;
...
inline Matrix4 Matrix4::CreateOrthographicOffCenter(float left, float right, float bottom, float top, float zNearPlane, float zFarPlane) noexcept
{
    using namespace DirectX;
    Matrix4 R;
...
inline Matrix4 Matrix4::CreateLookAt(const Vector3& eye, const Vector3& target, const Vector3& up) noexcept
{
    using namespace DirectX;
    Matrix4 R;
...
inline Matrix4 Matrix4::CreateWorld(const Vector3& position, const Vector3& forward, const Vector3& up) noexcept
{
    using namespace DirectX;
...
inline Matrix4 Matrix4::CreateFromQuaternion(const Quaternion& rotation) noexcept
{
    using namespace DirectX;
    Matrix4 R;
...
inline Matrix4 Matrix4::CreateFromYawPitchRoll(float yaw, float pitch, float roll) noexcept
{
    using namespace DirectX;
    Matrix4 R;
...
inline Matrix4 Matrix4::CreateShadow(const Vector3& lightDir, const Plane& plane) noexcept
{
    using namespace DirectX;
    Matrix4 R;
...
inline void Matrix4::Lerp(const Matrix4& M1, const Matrix4& M2, float t, Matrix4& result) noexcept
{
    using namespace DirectX;
    XMVECTOR x1 = XMLoadFloat4(reinterpret_cast<const XMFLOAT4*>(&M1._11));
    XMVECTOR x2 = XMLoadFloat4(reinterpret_cast<const XMFLOAT4*>(&M1._21));
...
inline Matrix4 Matrix4::Lerp(const Matrix4& M1, const Matrix4& M2, float t) noexcept
{
    using namespace DirectX;
    XMVECTOR x1 = XMLoadFloat4(reinterpret_cast<const XMFLOAT4*>(&M1._11));
    XMVECTOR x2 = XMLoadFloat4(reinterpret_cast<const XMFLOAT4*>(&M1._21));
...
inline void Matrix4::Transform(const Matrix4& M, const Quaternion& rotation, Matrix4& result) noexcept
{
    using namespace DirectX;
    XMVECTOR quatv = XMLoadFloat4(&rotation);
...
inline Matrix4 Matrix4::Transform(const Matrix4& M, const Quaternion& rotation) noexcept
{
    using namespace DirectX;
    XMVECTOR quatv = XMLoadFloat4(&rotation);
...
inline void Plane::Transform(const Plane& plane, const Matrix4& M, Plane& result) noexcept
{
    using namespace DirectX;
...
inline Vector3 Viewport::Project(const Vector3& p, const Matrix4& proj, const Matrix4& view, const Matrix4& world) const noexcept
{
    using namespace DirectX;
...
inline void Viewport::Project(const Vector3& p, const Matrix4& proj, const Matrix4& view, const Matrix4& world, Vector3& result) const noexcept
{
    using namespace DirectX;
    XMVECTOR v = XMLoadFloat3(&p);
    XMMATRIX projection = XMLoadFloat4x4(&proj);
...
inline Vector3 Viewport::Unproject(const Vector3& p, const Matrix4& proj, const Matrix4& view, const Matrix4& world) const noexcept
{
    using namespace DirectX;
    XMVECTOR v = XMLoadFloat3(&p);
    XMMATRIX projection = XMLoadFloat4x4(&proj);
...
inline void Viewport::Unproject(const Vector3& p, const Matrix4& proj, const Matrix4& view, const Matrix4& world, Vector3& result) const noexcept
{
    using namespace DirectX;
    XMVECTOR v = XMLoadFloat3(&p);
    XMMATRIX projection = XMLoadFloat4x4(&proj);

...
#define HZ_PROFILE 0
	...
	#define HZ_PROFILE_BEGIN_SESSION(name, filepath) ::gn::Instrumentor::Get().BeginSession(name, filepath)
	#define HZ_PROFILE_END_SESSION() ::gn::Instrumentor::Get().EndSession()
	#define HZ_PROFILE_SCOPE(name) ::gn::InstrumentationTimer timer##__LINE__(name);
